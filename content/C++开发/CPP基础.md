...
1. 第一优先级：作用域解析运算符 `::`
2. 第二优先级：结合运算符【函数调用、数组下标、取成员、类型转换、后置运算符等】【从左向右】。如 `()  []  ->  .  typeid  ++ --`【后缀】

3. 第三优先级：一元运算符。如：`!  ~  +  -  ++  --  &  *`  `()【类型转换】`  `sizeof new  new[]  delete  delete[]`
...
//多条件的if语句
if(条件1){ 条件1满足执行的语句 }
else if(条件2){条件2满足执行的语句}... else{ 都不满足执行的语句}
```
...
> [!attention] 注意：
> 
> 1. switch语句中表达式类型只能是**整型或者字符型**
> 2. case里如果没有break，那么程序会一直向下执行
> 3. c++17新增：`[[fallthrough]];` 用于替代不使用 `break` 的情况

> [!note] 总结
与if语句比，对于多条件判断时，switch的结构清晰，执行效率高，缺点是switch不可以判断区间。

> [!tip] if 与 switch 的效率差异
> if与switch的效率差异在于汇编不同。从汇编角度，switch将比较操作提前转换为一张字典表，每次执行swtich其实只是查表操作。
...
**执行逻辑**：起始表达式 -> 判断条件表达式 -> 执行循环内容 -> 执行末尾循环体 -> 判断……
...
> [!note] continue并没有使整个循环终止，而break会跳出循环
...
> [!attention] 在程序中不建议使用goto语句，以免造成程序流程混乱
...
1. 特点1：数组中的每个数据元素都是相同的数据类型。
2. 特点2：数组是由<span style="background:#FFFF00;">连续的内存</span>位置组成的。

> [!note] 变量不初始化编译器不让用，但数组不初始化是可以用的
>
...
> [!bug] 字符串赋值，如果需要二次处理，直接使用常量赋值的方法是不行的！
> 解决：需要重新分配内存空间，其中要注意返回值为局部变量的 "坑"！
...
> [!question] 提问：指针也是种数据类型，那么这种数据类型占用多少内存空间？
...
> [!note] 所有指针类型，在32位操作系统下是4个字节，在64位操作系统下是8个字节。
...
> [!warning] 空指针和野指针都不是我们申请的空间，因此不要访问。
...
> [!tip] 技巧：看const右侧紧跟着的是指针还是常量, 是指针就是常量指针，是常量就是指针常量
> 
...
> [!attention] ptr[5] 与 a[5] 原理一致，均是在基地址上进行偏移。

> [!attention] 注意：`sizeof(a)` 与 `sizeof(ptr)` 不同，前者是数组的大小，后者仅是指针的大小
...
> [!info] 如果不想修改实参，就用值传递，如果想修改实参，就用地址传递
...
1. 【直接打印`a`却输出 101，而非 200】C++ 编译器对`const`常量做了**常量折叠（Constant Folding）** 优化 —— 编译阶段直接将代码中所有引用`a`的地方替换为字面量 101，而非运行时从内存读取。
2. 【`*ptr`能读到 200】**指针修改的是内存，但不影响常量折叠**。
	- `int* ptr = (int*)&a`是强制类型转换，绕过了编译期的`const`检查，运行时确实修改了`a`对应的内存地址的值（`mov dword ptr [rax], 0C8h`），所以`*ptr`能读到 200；
...
```asm
; 原代码：const int a{ 101 }; 
mov dword ptr [a],65h ; 65h是101的十六进制，将101写入a的内存地址 ; 注意：a的内存地址是[rbp-8]（栈上） 

; 原代码：int* ptr = { (int*)&a }; 
lea rax,[a] ; 取a的内存地址，存入rax寄存器 
mov qword ptr [ptr],rax; 将地址赋值给ptr（ptr存在[rbp-10h]） 

; 原代码：cout << *ptr << ',' << ptr << endl; 
mov rax,qword ptr [ptr]; 取ptr的值（a的地址） 
mov ecx,dword ptr [rax]; 从a的内存地址读取值（此时是101），存入ecx ; 调用cout打印ecx的值（101），再打印ptr的地址 

; 原代码：*ptr = 200; 
mov rax,qword ptr [ptr]; 取ptr的值（a的地址） 
mov dword ptr [rax],0C8h; 0C8h是200的十六进制，写入a的内存地址 ; 此时a的内存地址的值已经被改成200 

; 原代码：cout << *ptr << ',' << ptr << endl; 
mov rax,qword ptr [ptr]; 取ptr的地址
 mov ecx,dword ptr [rax]; 从内存读取值（200），打印 

; 原代码：cout << a << endl; 
mov ecx,65h ; 重点！直接把101（65h）赋值给ecx ; 没有从a的内存地址读取，而是直接用字面量 ; 调用cout打印ecx的值（101）
```
...
> [!attention] 总结
> 1. **`const`的本质不是 “内存不可改”**：C++ 中的`const`是**编译期约束**，告诉编译器 “这个变量不应该被修改”，而非 “内存被保护不可写”。通过指针强制转换可以修改内存，但编译器的优化会让直接访问`const`变量的地方不受影响。
> 2. **这种操作是未定义行为**：C++ 标准规定，修改`const`限定的对象属于未定义行为，不同编译器（如 GCC、Clang）可能有不同表现（比如 GCC 可能也会输出 101，也可能因优化策略不同输出 200），实际开发中绝对不要这么写。
...
> **代码1**中，定义了一种简易的智能指针`SmartPtr`，在其析构函数中会对资源进行释放。因为申请的资源可能是通过`new T`、`new T[n]`、`malloc(...)`这几种方法的任意之一来申请的，每种方式申请的资源需要不同的关键字/函数来释放资源，否则程序可能会崩溃。

因此，需要一个模板参数`Del`，这个模板参数用于接收仿函数，以匹配不同的资源释放方式。【我们默认采用`delete`的方式释放资源，C++标准库中提供的智能指针，如果不显示给定仿函数来定义释放资源的方式，也是默认`delete`释放资源】。

**代码2**：智能指针的简易实现 -- `SmartPtr`
...
struct Delete
{
	void operator()(T* ptr) { delete ptr;} // 重载()，支持函数式调用
};
...
**代码2** 实现了一个基础的**独占式智能指针雏形**（类似`std::unique_ptr`），核心能力是：

1. 用模板类`SmartPtr`管理任意类型的裸指针（`T*`）；
2. 支持自定义删除器（Del），适配`delete`、`delete[]`、`free`等不同的资源释放方式；
3. 利用析构函数自动调用删除器，避免内存泄漏（栈上的`SmartPtr`对象销毁时，自动释放堆上的资源）。

> [!info] 自定义删除器（核心辅助结构）
> 
> 删除器的作用是**将 “资源释放逻辑” 和 “指针管理逻辑” 解耦**，适配不同的内存分配方式（`new`/`new[]`/`malloc`）

> [!info] 代码中用`struct`定义删除器（如`Delete`/`DeleteArray`/`Free`），核心原因是：
> 
> 1. `struct`的**默认访问权限是 public**，无需额外写`public`修饰符，能让`operator()`直接被外部调用（如`SmartPtr`的析构函数中调用`del(_ptr)`）；
> 2. 这类删除器是 “纯功能型轻量级结构”，`struct`语义上更贴合 “数据 / 行为简单聚合” 的场景；
> 3. 和`class`在功能上完全等价（仅默认权限不同），换成`class`并显式加`public`也能实现相同效果。
...
using namespace std;
int main() { 
	// 1. 管理单个对象（new分配，默认删除器Delete<int>） 
	SmartPtr<int> sp1(new int(10)); // sp1析构时，自动调用Delete<int>()(ptr) → delete ptr 
	
	// 2. 管理数组（new[]分配，指定DeleteArray<int>删除器） 
	SmartPtr<int, DeleteArray<int>> sp2(new int[5]); // sp2析构时，自动调用DeleteArray<int>()(ptr) → delete[] ptr 
	// 3. 管理C风格内存（malloc分配，指定Free<int>删除器） 
	int* p = (int*)malloc(sizeof(int)); 
	*p = 20; 
	SmartPtr<int, Free<int>> sp3(p); // sp3析构时，自动调用Free<int>()(ptr) → free(ptr) 
...
但是值拷贝会存在对同一块空间多次释放的问题，对此，C++标准库中的智能指针`auto_ptr`和`shared_ptr`分别采用了管理权转移和引用计数的方法来解决问题，但一般会通过 **引用计数** 解决多次释放的问题。
...
> [!note] 如果不想修改主函数中的数据，用值传递，反之用地址传递
...
实际上从内存的角度，**数组是若干个连续的相同大小元素的集合**。

也就是说，`char[20]` 和 连续的20 个char 元素在内存中没有什么分别。所以当结构体中有数组时，可以把它拆解开成连续排列的多个子元素。
...
2. 性能原因，经过内存对齐后，CPU的访问效率会得到很大的提高。
