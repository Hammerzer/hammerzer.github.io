---
title: CPP基础练习项目
date: 2024-03-13 17:09:53
urlname: cpp-base-project
tags:
  - CPP
  - 基础知识
  - 项目
categories: CPP
description: 两个C++的基础练习。
draft: false
---
...
>
> - 演讲比赛管理系统
> - 机房管理系统    <span style="background:#FFFF00;"></span>
...
比赛流程分析：
1. 抽签 → 开始演讲比赛 → 显示第一轮比赛结果 → 
2. 抽签 → 开始演讲比赛 → 显示前三名结果 → 保存分数
...
* 在`speechManager.cpp`中将startSpeech的空实现先写入
* 我们可以先将整个比赛的流程 写到函数中
...
void SpeechManager::speechContest(
{
	...
	for (vector<int>::iterator it = v_Src.begin(; it != v_Src.end(; it++
	{
		...
		double avg = sum / (doubled.size(;									//获取平均分
...
* 在SpeechManager构造函数中调用获取往届记录函数【初始载入往届信息】
...
![](baseProj-cpp-2-14.png)

3. 比完赛后查不到本届比赛的记录，没有实时更新

解决方式：比赛完毕后，所有数据重置

![](baseProj-cpp-2-15.png)
...
void SpeechManager::speechContest(
{
	...
	for (vector<int>::iterator it = v_Src.begin(; it != v_Src.end(; it++
	{
		...
		double avg = sum / (doubled.size(;  //平均分
...
- 在机房预约系统`.cpp`中，当用户登录的是教师，添加教师菜单接口
- 将不同的分支提供出来
  - 查看所有预约
  - 审核预约
  - 注销登录
