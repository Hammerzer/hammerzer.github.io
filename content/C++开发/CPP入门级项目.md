...
1. 互斥体：lock/unlock
	1. 注意：该同步操作依赖程序员代码配合
	2. 实际上将多任务变为随机的排队执行。
	3. 策略：①权重竞争；②随机竞争：出现性能抖动。
	4. 解决：消息机制。如SendMessage。
		- 但是有性能极限，比如30ms延迟。
		- 且必须依赖与消息的传参规范。
		- 属于伪并发，消息处理也是一个一个处理的。
2. 网络：回环网络（本机网络）127.0.0.1
	1. 优势：
		1. 速度快（千兆网卡1000Mb，即125MB/s）；
		2. 一对多：可以一个服务支持多个链接，无须程序员关心等待队列；
		3. Windows的IOCP、Linux的epoll，提高效率，至少支持10000并发。
		4. 这也是集群的基础，本身的线程同步也是通过网络实现，集群所需的也是网络。
...
1. 创建完成端口映射对象（由操作系统接管），返回一个句柄（用于沟通内核）
2. 新建一个线程，用于读取信息，并操作队列
